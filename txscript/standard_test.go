...
// newAddressPubKey returns a new bteutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) bteutil.Address {
	addr, err := bteutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new bteutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) bteutil.Address {
	addr, err := bteutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new bteutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) bteutil.Address {
	addr, err := bteutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// newAddressTaproot returns a new bteutil.AddressTaproot from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressTaproot(scriptHash []byte) bteutil.Address {
	addr, err := bteutil.NewAddressTaproot(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
		panic("invalid script hash in test source")
	}
...
func TestExtractPkScriptAddrs(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			addrs: []bteutil.Address{newAddressTaproot(
...
// bogusAddress implements the bteutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the bteutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := bteutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := bteutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := bteutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := bteutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := bteutil.NewAddressPubKey(hexToBytes("0411"+
	...
	p2wsh, err := bteutil.NewAddressWitnessScriptHash(hexToBytes("e981bd992a43650657"+
	...
	p2tr, err := bteutil.NewAddressTaproot(hexToBytes("3a8e170b546c3b122ab9c175e"+
	...
	p2wpkh, err := bteutil.NewAddressWitnessPubKeyHash(hexToBytes("748e50366adb8"+
	...
	tests := []struct {
		...
		{(*bteutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*bteutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*bteutil.AddressPubKey)(nil), "", errUnsupportedAddress},
		{(*bteutil.AddressWitnessPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*bteutil.AddressWitnessScriptHash)(nil), "", errUnsupportedAddress},
		{(*bteutil.AddressTaproot)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*bteutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*bteutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
