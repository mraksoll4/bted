...
func TestBlock(t *testing.T) {
	b := bteutil.NewBlock(&Block100000)
...
func TestNewBlockFromBytes(t *testing.T) {
	...
	b, err := bteutil.NewBlockFromBytes(block100000Bytes)
	if err != nil {
...
func TestNewBlockFromBlockAndBytes(t *testing.T) {
	...
	// Create a new block from the serialized bytes.
	b := bteutil.NewBlockFromBlockAndBytes(&Block100000, block100000Bytes)
...
func TestBlockErrors(t *testing.T) {
	...
	testErr := bteutil.OutOfRangeError(wantErr)
	...
	_, err = bteutil.NewBlockFromBytes(shortBytes)
	if err != io.EOF {
	...
	_, err = b.TxHash(-1)
	if _, ok := err.(bteutil.OutOfRangeError); !ok {
		t.Errorf("TxHash: wrong error - got: %v <%T>, "+
			"want: <%T>", err, err, bteutil.OutOfRangeError(""))
	}
