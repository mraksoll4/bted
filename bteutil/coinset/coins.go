...
func (cs *CoinSet) TotalValue() (value bteutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue bteutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue bteutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount bteutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue bteutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue bteutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue bteutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue bteutil.Amount, coins []Coin) (Coins, error) {
...
// bteutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() bteutil.Amount {
	return bteutil.Amount(c.txOut().Value)
}
