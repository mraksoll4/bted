...
func TestAddresses(t *testing.T) {
	tests := []struct {
		...
		encoded string
		valid   bool
		result  bteutil.Address
		f       func() (bteutil.Address, error)
		net     *chaincfg.Params
	}{
		// Positive P2PKH tests.
		{
			...
			valid:   true,
			result: bteutil.TstAddressPubKeyHash(
				[ripemd160.Size]byte{
			...
			f: func() (bteutil.Address, error) {
				pkHash := []byte{
					0xe3, 0x4c, 0xce, 0x70, 0xc8, 0x63, 0x73, 0x27, 0x3e, 0xfc,
					0xc5, 0x4c, 0xe7, 0xd2, 0xa4, 0x91, 0xbb, 0x4a, 0x0e, 0x84}
				return bteutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
			},
		...
		},
		{
			...
			f: func() (bteutil.Address, error) {
				pkHash := []byte{
					0x13, 0xc6, 0x0d, 0x8e, 0x68, 0xd7, 0x34, 0x9f, 0x5b, 0x4c,
					0xa3, 0x62, 0xc3, 0x95, 0x4b, 0x15, 0x04, 0x50, 0x61, 0xb1}
				return bteutil.NewAddressPubKeyHash(pkHash, &customParams)
			},
			net: &customParams,
		},
		{
			...
			f: func() (bteutil.Address, error) {
				pkHash := []byte{
					0x78, 0xb3, 0x16, 0xa0, 0x86, 0x47, 0xd5, 0xb7, 0x72, 0x83,
					0xe5, 0x12, 0xd3, 0x60, 0x3f, 0x1f, 0x1c, 0x8d, 0xe6, 0x8f}
				return bteutil.NewAddressPubKeyHash(pkHash, &chaincfg.TestNet3Params)
			},
		...
		// Positive P2SH tests.
		{
			...
			valid:   true,
			result: bteutil.TstAddressScriptHash(
				[ripemd160.Size]byte{
			...
			f: func() (bteutil.Address, error) {
				script := []byte{
					...
					0xae}
				return bteutil.NewAddressScriptHash(script, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
			...
			f: func() (bteutil.Address, error) {
				pkHash := []byte{
					0xEE, 0x34, 0xAC, 0x67, 0x6B, 0xDA, 0xF6, 0xE3, 0x70, 0xC8,
					0xC8, 0x20, 0xB9, 0x48, 0xED, 0xFA, 0xD3, 0xA8, 0x73, 0xD8}
				return bteutil.NewAddressScriptHashFromHash(pkHash, &customParams)
			},
			net: &customParams,
		},
		{
			...
			f: func() (bteutil.Address, error) {
				hash := []byte{
					0xe8, 0xc3, 0x00, 0xc8, 0x79, 0x86, 0xef, 0xa8, 0x4c, 0x37,
					0xc0, 0x51, 0x99, 0x29, 0x01, 0x9e, 0xf8, 0x6e, 0xb5, 0xb4}
				return bteutil.NewAddressScriptHashFromHash(hash, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
			...
			f: func() (bteutil.Address, error) {
				hash := []byte{
					0xc5, 0x79, 0x34, 0x2c, 0x2c, 0x4c, 0x92, 0x20, 0x20, 0x5e,
					0x2c, 0xdc, 0x28, 0x56, 0x17, 0x04, 0x0c, 0x92, 0x4a, 0x0a}
				return bteutil.NewAddressScriptHashFromHash(hash, &chaincfg.TestNet3Params)
			},
			net: &chaincfg.TestNet3Params,
		},
		...
		// Positive P2PK tests.
		{
			...
			valid:   true,
			result: bteutil.TstAddressPubKey(
				[]byte{
					0x02, 0x19, 0x2d, 0x74, 0xd0, 0xcb, 0x94, 0x34, 0x4c, 0x95,
					0x69, 0xc2, 0xe7, 0x79, 0x01, 0x57, 0x3d, 0x8d, 0x79, 0x03,
					0xc3, 0xeb, 0xec, 0x3a, 0x95, 0x77, 0x24, 0x89, 0x5d, 0xca,
					0x52, 0xc6, 0xb4},
				bteutil.PKFCompressed, chaincfg.MainNetParams.PubKeyHashAddrID),
			f: func() (bteutil.Address, error) {
				serializedPubKey := []byte{
					...
					0xc3, 0xeb, 0xec, 0x3a, 0x95, 0x77, 0x24, 0x89, 0x5d, 0xca,
					0x52, 0xc6, 0xb4}
				return bteutil.NewAddressPubKey(serializedPubKey, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
		...
		},
		{
			...
			result: bteutil.TstAddressPubKey(
				...
				bteutil.PKFUncompressed, chaincfg.MainNetParams.PubKeyHashAddrID),
			f: func() (bteutil.Address, error) {
				serializedPubKey := []byte{
					0x04, 0x11, 0xdb, 0x93, 0xe1, 0xdc, 0xdb, 0x8a, 0x01, 0x6b,
					0x49, 0x84, 0x0f, 0x8c, 0x53, 0xbc, 0x1e, 0xb6, 0x8a, 0x38,
		...
		},
		{
			...
			result: bteutil.TstAddressPubKey(
				[]byte{
					...
					0xc3, 0xeb, 0xec, 0x3a, 0x95, 0x77, 0x24, 0x89, 0x5d, 0xca,
					0x52, 0xc6, 0xb4},
				bteutil.PKFCompressed, chaincfg.TestNet3Params.PubKeyHashAddrID),
			f: func() (bteutil.Address, error) {
				serializedPubKey := []byte{
					0x02, 0x19, 0x2d, 0x74, 0xd0, 0xcb, 0x94, 0x34, 0x4c, 0x95,
					0x69, 0xc2, 0xe7, 0x79, 0x01, 0x57, 0x3d, 0x8d, 0x79, 0x03,
					0xc3, 0xeb, 0xec, 0x3a, 0x95, 0x77, 0x24, 0x89, 0x5d, 0xca,
					0x52, 0xc6, 0xb4}
				return bteutil.NewAddressPubKey(serializedPubKey, &chaincfg.TestNet3Params)
			},
			net: &chaincfg.TestNet3Params,
		},
		{
		...
		},
		{
			...
			result: bteutil.TstAddressPubKey(
				[]byte{
					...
					0x8b, 0x64, 0xf9, 0xd4, 0xc0, 0x3f, 0x99, 0x9b, 0x86, 0x43,
					0xf6, 0x56, 0xb4, 0x12, 0xa3},
				bteutil.PKFUncompressed, chaincfg.TestNet3Params.PubKeyHashAddrID),
			f: func() (bteutil.Address, error) {
				serializedPubKey := []byte{
					0x04, 0x11, 0xdb, 0x93, 0xe1, 0xdc, 0xdb, 0x8a, 0x01, 0x6b,
					0x49, 0x84, 0x0f, 0x8c, 0x53, 0xbc, 0x1e, 0xb6, 0x8a, 0x38,
		...
		// Segwit address tests.
		{
			...
			valid:   true,
			result: bteutil.TstAddressWitnessPubKeyHash(
				0,
				[20]byte{
			...
			f: func() (bteutil.Address, error) {
				pkHash := []byte{
					0x75, 0x1e, 0x76, 0xe8, 0x19, 0x91, 0x96, 0xd4, 0x54, 0x94,
					0x1c, 0x45, 0xd1, 0xb3, 0xa3, 0x23, 0xf1, 0x43, 0x3b, 0xd6}
				return bteutil.NewAddressWitnessPubKeyHash(pkHash, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
			...
			valid:   true,
			result: bteutil.TstAddressWitnessScriptHash(
				0,
				[32]byte{
			...
			f: func() (bteutil.Address, error) {
				scriptHash := []byte{
					...
					0x6c, 0x98, 0x56, 0x78, 0xcd, 0x4d, 0x27, 0xa1,
					0xb8, 0xc6, 0x32, 0x96, 0x04, 0x90, 0x32, 0x62}
				return bteutil.NewAddressWitnessScriptHash(scriptHash, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
			...
			f: func() (bteutil.Address, error) {
				pkHash := []byte{
					0x75, 0x1e, 0x76, 0xe8, 0x19, 0x91, 0x96, 0xd4, 0x54, 0x94,
					0x1c, 0x45, 0xd1, 0xb3, 0xa3, 0x23, 0xf1, 0x43, 0x3b, 0xd6}
				return bteutil.NewAddressWitnessPubKeyHash(pkHash, &chaincfg.TestNet3Params)
			},
			net: &chaincfg.TestNet3Params,
		},
		{
			...
			f: func() (bteutil.Address, error) {
				scriptHash := []byte{
					...
					0x6c, 0x98, 0x56, 0x78, 0xcd, 0x4d, 0x27, 0xa1,
					0xb8, 0xc6, 0x32, 0x96, 0x04, 0x90, 0x32, 0x62}
				return bteutil.NewAddressWitnessScriptHash(scriptHash, &chaincfg.TestNet3Params)
			},
			net: &chaincfg.TestNet3Params,
		},
		{
		...
		},
		{
			...
			f: func() (bteutil.Address, error) {
				pkHash := []byte{
					0x75, 0x1e, 0x76, 0xe8, 0x19, 0x91, 0x96, 0xd4, 0x54, 0x94,
					0x1c, 0x45, 0xd1, 0xb3, 0xa3, 0x23, 0xf1, 0x43, 0x3b, 0xd6}
				return bteutil.NewAddressWitnessPubKeyHash(pkHash, &customParams)
			},
			net: &customParams,
		},
		...
		// P2TR address tests.
		{
			...
			valid:   true,
			result: bteutil.TstAddressTaproot(
				1, [32]byte{
			...
			f: func() (bteutil.Address, error) {
				...
				return bteutil.NewAddressTaproot(
	...
	for _, test := range tests {
		...
		decoded, err := bteutil.DecodeAddress(test.addr, test.net)
		...
		if err == nil {
			...
			case *bteutil.AddressPubKeyHash:
				saddr = bteutil.TstAddressSAddr(encoded)

			case *bteutil.AddressScriptHash:
				saddr = bteutil.TstAddressSAddr(encoded)

			case *bteutil.AddressPubKey:
				// Ignore the error here since the script
				// address is checked below.
				saddr, _ = hex.DecodeString(d.String())
			case *bteutil.AddressWitnessPubKeyHash:
				saddr = bteutil.TstAddressSegwitSAddr(encoded)
			case *bteutil.AddressWitnessScriptHash:
				saddr = bteutil.TstAddressSegwitSAddr(encoded)
			case *bteutil.AddressTaproot:
				saddr = bteutil.TstAddressTaprootSAddr(encoded)
			}
			...
			case *bteutil.AddressWitnessPubKeyHash:
				...
				expVer := test.result.(*bteutil.AddressWitnessPubKeyHash).WitnessVersion()
			...
			case *bteutil.AddressWitnessScriptHash:
				...
				expVer := test.result.(*bteutil.AddressWitnessScriptHash).WitnessVersion()
				if v := a.WitnessVersion(); v != expVer {
					t.Errorf("%v: witness versions do not match:\n%x != \n%x",
						test.name, expVer, v)
					return
