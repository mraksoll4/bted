...
func TestNewTxFromBytes(t *testing.T) {
	...
	tx, err := bteutil.NewTxFromBytes(testTxBytes)
	if err != nil {
...
func TestTxErrors(t *testing.T) {
	...
	_, err = bteutil.NewTxFromBytes(shortBytes)
	if err != io.EOF {
...
func TestTxHasWitness(t *testing.T) {
	...
	tx := bteutil.NewTx(msgTx)

	tx.WitnessHash() // Populate the witness hash cache
	tx.HasWitness()  // Should not fail (see btcsuite/bted#1543)
	...
	for _, msgTxWithoutWitness := range Block100000.Transactions[1:] {
		txWithoutWitness := bteutil.NewTx(msgTxWithoutWitness)
