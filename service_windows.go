...
const (
	// svcName is the name of bted service.
	svcName = "btedsvc"
...
// logServiceStartOfDay logs information about bted when the main server has
...
// btedService houses the main service handler which handles all service
// updates and launching btedMain.
type btedService struct{}
...
// long-running btedMain (which is the real meat of bted), handles service
// change requests, and notifies the service control manager of changes.
func (s *btedService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start btedMain in a separate goroutine so the service can start
...
// installService attempts to install the bted service.  Typically this should
...
// removeService attempts to uninstall the bted service.  Typically this should
...
// startService attempts to start the bted service.
...
func serviceMain() (bool, error) {
	...
	// needed to run bted on Windows in CI environments like Travis.
	...
	err = svc.Run(svcName, &btedService{})
	if err != nil {
