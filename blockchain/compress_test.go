...
func TestAmountCompression(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "0 BTE (sometimes used in nulldata)",
			uncompressed: 0,
			compressed:   0,
		},
		{
		...
		{
			name:         "0.00001 BTE (typical transaction fee)",
			uncompressed: 1000,
			compressed:   4,
		},
		{
			name:         "0.0001 BTE (typical transaction fee)",
			uncompressed: 10000,
			compressed:   5,
		},
		{
		...
		{
			name:         "21000000 BTE (max minted coins)",
...
func TestCompressedTxOut(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "pay-to-pubkey uncompressed 1 BTE",
			amount:     100000000,
