...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: bteutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := bteutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*bteutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, bteutil.NewTx(tx))
	}
...
func additionalCoinbase(amount bteutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee bteutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee bteutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee bteutil.Amount) *wire.MsgTx {
...
func encodeNonCanonicalBlock(b *wire.MsgBlock) []byte {
	...
	b.Header.BteEncode(&buf, 0, wire.BaseEncoding)
	buf.Write(nonCanonicalVarInt(uint32(len(b.Transactions))))
	for _, tx := range b.Transactions {
		tx.BteEncode(&buf, 0, wire.BaseEncoding)
	}
	return buf.Bytes()
}
