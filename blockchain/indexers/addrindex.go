...
func addrToKey(addr bteutil.Address) ([addrKeySize]byte, error) {
	...
	case *bteutil.AddressWitnessScriptHash:
		...
		// push to 20-bytes.
		copy(result[1:], bteutil.Hash160(addr.ScriptAddress()))
		return result, nil
...
type AddrIndex struct {
	...
	txnsByAddr      map[[addrKeySize]byte]map[chainhash.Hash]*bteutil.Tx
...
// the passed map.
func (idx *AddrIndex) indexBlock(data writeIndexData, block *bteutil.Block,
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) ConnectBlock(dbTx database.Tx, block *bteutil.Block,
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) DisconnectBlock(dbTx database.Tx, block *bteutil.Block,
	stxos []blockchain.SpentTxOut) error {
...
func (idx *AddrIndex) TxRegionsForAddress(dbTx database.Tx, addr bteutil.Address,
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) indexUnconfirmedAddresses(pkScript []byte, tx *bteutil.Tx) {
	...
	for _, addr := range addresses {
		...
		if addrIndexEntry == nil {
			addrIndexEntry = make(map[chainhash.Hash]*bteutil.Tx)
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) AddUnconfirmedTx(tx *bteutil.Tx, utxoView *blockchain.UtxoViewpoint) {
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) UnconfirmedTxnsForAddress(addr bteutil.Address) []*bteutil.Tx {
	// Ignore unsupported address types.
	addrKey, err := addrToKey(addr)
	if err != nil {
		return nil
	...
	if txns, exists := idx.txnsByAddr[addrKey]; exists {
		addressTxns := make([]*bteutil.Tx, 0, len(txns))
		for _, tx := range txns {
...
func NewAddrIndex(db database.DB, chainParams *chaincfg.Params) *AddrIndex {
	return &AddrIndex{
		...
		txnsByAddr:  make(map[[addrKeySize]byte]map[chainhash.Hash]*bteutil.Tx),
