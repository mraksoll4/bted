...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *bteutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *bteutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *bteutil.Tx, isNew bool) {
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a bteutil.Address) {
	switch a := a.(type) {
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := bteutil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a bteutil.Address) bool {
	switch a := a.(type) {
	case *bteutil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a bteutil.Address) {
	switch a := a.(type) {
	case *bteutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected bteutil.Block
type notificationBlockDisconnected bteutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *bteutil.Tx,
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *bteutil.Block) {
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *bteutil.Tx) {
	...
	ntfn := btcjson.NewTxAcceptedNtfn(txHashStr, bteutil.Amount(amount).ToBTE())
	marshalledJSON, err := btcjson.MarshalCmd(btcjson.RpcVersion1, nil, ntfn)
	if err != nil {
...
func (m *wsNotificationManager) addSpentRequests(opMap map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	spends := make(map[chainhash.Hash]*bteutil.Tx)
	for _, op := range ops {
...
// blockDetails creates a BlockDetails struct to include in btews notifications
// from a block and a transaction's block index.
func blockDetails(block *bteutil.Block, txIndex int) *btcjson.BlockDetails {
	if block == nil {
		return nil
	}
...
func newRedeemingTxNotification(txHex string, index int, block *bteutil.Block) ([]byte, error) {
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *bteutil.Tx, block *bteutil.Block) {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *bteutil.Tx,
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *bteutil.Tx, block *bteutil.Block) {
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := bteutil.DecodeAddress(addr, params)
		if err != nil {
...
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *bteutil.Block) {
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *bteutil.Block, params *chaincfg.Params) []string {
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *bteutil.Block) error {
...
func scanBlockChunks(wsc *wsClient, cmd *btcjson.RescanCmd, lookups *rescanKeys, minBlock,
	...
	*bteutil.Block, *chainhash.Hash, error) {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	// Notify websocket client of the finished rescan.  Due to how bted
