...
Bitcoin RPC server that uses a bted/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with bted (https://github.com/btcsuite/bted),
btewallet (https://github.com/btcsuite/btewallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, bted and
btewallet provide a websocket interface that is more efficient than the standard
...
bted or btewallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by bted and
btewallet only uses a single connection that remains open and allows
...
All notifications provided by bted require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by bted (and
btewallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that bted intentionally separates the wallet
functionality into a separate process named btewallet.  This means if you are
connected to the bted RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, btewallet provides pass through treatment
...
The following full-blown client examples are in the examples directory:
 ...
 - btedwebsockets
   ...
   block count
 - btewalletwebsockets
   Connects to a btewallet RPC server using TLS-secured websockets, registers
