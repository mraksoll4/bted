...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *bteutil.Tx, details *btcjson.BlockDetails)
	...
	//
	// Deprecated: Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *bteutil.Tx, details *btcjson.BlockDetails)
	...
	// result of a rescan request, due to how bted may send various rescan
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount bteutil.Amount)
	...
	// OnBtedConnected is invoked when a wallet connects or disconnects from
	// bted.
	//
	// This will only be available when client is connected to a wallet
	// server such as btewallet.
	OnBtedConnected func(connected bool)

	// OnAccountBalance is invoked with account balance updates.
	//
	// This will only be available when speaking to a wallet server
	// such as btewallet.
	OnAccountBalance func(account string, balance bteutil.Amount, confirmed bool)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	// OnBtedConnected
	case btcjson.BtedConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
		if c.ntfnHandlers.OnBtedConnected == nil {
			return
		}

		connected, err := parseBtedConnectedNtfnParams(ntfn.Params)
		if err != nil {
			log.Warnf("Received invalid bted connected "+
				"notification: %v", err)
			return
		}

		c.ntfnHandlers.OnBtedConnected(connected)
...
//
// NOTE: This is a bted extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*bteutil.Tx, error) {
	...
	transactions := make([]*bteutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = bteutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*bteutil.Tx,
	*btcjson.BlockDetails, error) {
	...
	return bteutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := bteutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
// parseBtedConnectedNtfnParams parses out the connection status of bted
// and btewallet from the parameters of a btedconnected notification.
func parseBtedConnectedNtfnParams(params []json.RawMessage) (bool, error) {
	if len(params) != 1 {
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance bteutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := bteutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
//
// NOTE: This is a bted extension and requires a websocket connection.
...
//
// Deprecated: Use LoadTxFilterAsync instead.
func (c *Client) NotifyReceivedAsync(addresses []bteutil.Address) FutureNotifyReceivedResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
...
//
// Deprecated: Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []bteutil.Address) error {
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []bteutil.Address, outpoints []*wire.OutPoint,
...
// NOTE: This is a bted extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []bteutil.Address,
...
// NOTE: This is a bted extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []bteutil.Address, outPoints []wire.OutPoint) error {
