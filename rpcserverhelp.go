...
var helpDescsEnUS = map[string]string{
	...
	"debuglevel--synopsis": "Dynamically changes the debug logging level.\n" +
		...
		"The valid subsystems are AMGR, ADXR, BCDB, BMGR, BTED, CHAN, DISC, PEER, RPCS, SCRP, SRVR, and TXMP.\n" +
	...
	"createrawtransaction-amounts--desc":  "The destination address as the key and the amount in BTE as the value",
	...
	"infochainresult-relayfee":        "The minimum relay fee for non-free transactions in BTE/KB",
	...
	"infowalletresult-paytxfee":        "The transaction fee set in BTE/KB",
	"infowalletresult-relayfee":        "The minimum relay fee for non-free transactions in BTE/KB",
	...
	"gettxoutresult-value":         "The transaction amount in BTE",
	...
	"sendrawtransaction-feesetting":   "Whether or not to allow insanely high fees in bitcoind < v0.19.0 or the max fee rate for bitcoind v0.19.0 and later (bted does not yet implement this parameter, so it has no effect)",
	...
	// StopCmd help.
	"stop--synopsis": "Shutdown bted.",
	"stop--result0":  "The string 'bted stopping.'",
	...
	// NotifySpentCmd help.
	"notifyspent--synopsis": "Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this bted instance) and when such a transaction first appears in a newly-attached block.",
