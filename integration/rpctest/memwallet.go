...
type memWallet struct {
	...
	addrs map[uint32]bteutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]bteutil.Address)
...
// SetRPCClient saves the passed rpc connection to bted as the wallet's
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*bteutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          bteutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (bteutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []bteutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt bteutil.Amount,
	feeRate bteutil.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := bteutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	...
	var outputAmt bteutil.Amount
	for _, output := range outputs {
		outputAmt += bteutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() bteutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance bteutil.Amount
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (bteutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := bteutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
