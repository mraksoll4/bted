...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt bteutil.Amount) *chainhash.Hash {
		...
		// Next, send amt BTE to this address, spending from one of our mature
		// coinbase outputs.
	...
	// input.
	txid := genSpend(bteutil.Amount(5 * bteutil.SatoshiPerBitcoin))
	...
	txid = genSpend(bteutil.Amount(500 * bteutil.SatoshiPerBitcoin))
	blockHashes, err = r.Client.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(bteutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*bteutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, bteutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	// The internal wallet of this harness should now have 250 BTE.
	expectedBalance := bteutil.Amount(250 * bteutil.SatoshiPerBitcoin)
	...
	// The original wallet should now have a balance of 0 BTE as its entire
	// chain should have been decimated in favor of the main harness'
	// chain.
	expectedBalance = bteutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := bteutil.Amount(50 * bteutil.SatoshiPerBitcoin)
	...
	// The current wallet balance should now be at least 50 BTE less
...
func TestHarness(t *testing.T) {
	// We should have (numMatureOutputs * 50 BTE) of mature unspendable
	// outputs.
	expectedBalance := bteutil.Amount(numMatureOutputs * 50 * bteutil.SatoshiPerBitcoin)
