...
// Harness fully encapsulates an active bted process to provide a unified
// platform for creating rpc driven integration tests involving bted. The
// active bted node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active bted process is fully
...
// used. If a custom bted executable is specified, it will be used to start the
...
func New(activeNet *chaincfg.Params, handlers *rpcclient.NotificationHandlers,
	...
	if handlers.OnFilteredBlockConnected != nil {
		...
		handlers.OnFilteredBlockConnected = func(height int32, header *wire.BlockHeader, filteredTxns []*bteutil.Tx) {
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the bted node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []bteutil.Address{h.wallet.coinbaseAddr}
	if err := h.Client.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure bted properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created bted
...
func (h *Harness) NewAddress() (bteutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() bteutil.Amount {
...
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut,
	feeRate bteutil.Amount) (*chainhash.Hash, error) {
...
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut,
	feeRate bteutil.Amount, change bool) (*wire.MsgTx, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) GenerateAndSubmitBlock(txns []*bteutil.Tx, blockVersion int32,
	blockTime time.Time) (*bteutil.Block, error) {
...
func (h *Harness) GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
	txns []*bteutil.Tx, blockVersion int32, blockTime time.Time,
	mineTo []wire.TxOut) (*bteutil.Block, error) {
	...
	prevBlock := bteutil.NewBlock(mBlock)
...
func baseDir() (string, error) {
	dirPath := filepath.Join(os.TempDir(), "bted", "rpctest")
