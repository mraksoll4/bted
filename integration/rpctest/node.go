...
func newConfig(prefix, certFile, keyFile string, extra []string,
	...
	var btedPath string
	if customExePath != "" {
		btedPath = customExePath
	} else {
		var err error
		btedPath, err = btcdExecutablePath()
		if err != nil {
			btedPath = "bted"
		}
	}
...
// arguments returns an array of arguments that be used to launch the bted
// process.
...
// command returns the exec.Cmd which will be used to start the bted process.
...
// to the bted process that is launched via Start().
...
// as the base for the log and data directories for bted.
...
// start creates a new bted process, and writes its pid in a file reserved for
...
// stop interrupts the running bted process process, and waits until it exits
...
// shutdown terminates the running bted process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := bteutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
