...
// oldBtedHomeDir returns the OS specific home directory bted used prior to
// version 0.3.3.  This has since been replaced with bteutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldBtedHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "bted")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".bted")
	}
...
// location prior to bted version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to bted
...
func upgradeDBPaths() error {
	...
	oldDbRoot := filepath.Join(oldBtedHomeDir(), "db")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "bted.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "bted_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "bted_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to bted
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		btedLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old bted.conf into new location if needed.
		...
		if ohpEmpty {
			...
			btedLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to bted as new versions require it.
