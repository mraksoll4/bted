...
func (msg *MsgCFHeaders) BteDecode(r io.Reader, pver uint32, _ MessageEncoding) error {
	// Read filter type
	...
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			...
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.BteDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgCFHeaders) BteEncode(w io.Writer, pver uint32, _ MessageEncoding) error {
	// Write filter type
	...
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			"message [count %v, max %v]", count,
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.BteEncode", str)
	}
...
// differs from BteDecode in that BteDecode decodes from the bitcoin wire
