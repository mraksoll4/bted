...
func TestMsgAlert(t *testing.T) {
	...
	// Test BteEncode with Payload == nil
	var buf bytes.Buffer
	err := msg.BteEncode(&buf, pver, encoding)
	if err != nil {
		t.Error(err.Error())
	}
	...
	if !bytes.Equal(buf.Bytes(), expectedBuf) {
		t.Errorf("BteEncode got: %s want: %s",
...
func TestMsgAlertWire(t *testing.T) {
	...
	for i, test := range tests {
		...
		var buf bytes.Buffer
		err := test.in.BteEncode(&buf, test.pver, test.enc)
		if err != nil {
			t.Errorf("BteEncode #%d error %v", i, err)
			continue
		}
		if !bytes.Equal(buf.Bytes(), test.buf) {
			t.Errorf("BteEncode #%d\n got: %s want: %s", i,
...
func TestMsgAlertWireErrors(t *testing.T) {
	...
	for i, test := range tests {
		...
		err := test.in.BteEncode(w, test.pver, test.enc)
		if reflect.TypeOf(err) != reflect.TypeOf(test.writeErr) {
			t.Errorf("BteEncode #%d wrong error got: %v, want: %v",
				i, err, test.writeErr)
			continue
		}
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.writeErr {
				t.Errorf("BteEncode #%d wrong error got: %v, "+
	...
	w := new(bytes.Buffer)
	err := baseMsgAlert.BteEncode(w, pver, encoding)
	if _, ok := err.(*MessageError); !ok {
		t.Errorf("MsgAlert.BteEncode wrong error got: %T, want: %T",
			err, MessageError{})
	}
	...
	buf := *new(bytes.Buffer)
	err = baseMsgAlert.BteEncode(&buf, pver, encoding)
	if _, ok := err.(*MessageError); !ok {
		t.Errorf("MsgAlert.BteEncode wrong error got: %T, want: %T",
			err, MessageError{})
	}
