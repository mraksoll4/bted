...
func TestCalcMinRequiredTxRelayFee(t *testing.T) {
	tests := []struct {
		...
		relayFee bteutil.Amount // minimum relay transaction fee.
...
func TestDust(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wire.TxOut{Value: bteutil.MaxSatoshi, PkScript: pkScript},
			bteutil.MaxSatoshi,
			false,
		},
		{
...
func TestCheckTransactionStandard(t *testing.T) {
	...
	addr, err := bteutil.NewAddressPubKeyHash(addrHash[:],
	...
	for _, test := range tests {
		...
		err := CheckTransactionStandard(bteutil.NewTx(&test.tx),
