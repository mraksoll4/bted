...
type Config struct {
	...
	FetchUtxoView func(*bteutil.Tx) (*blockchain.UtxoViewpoint, error)
	...
	// utxo view.
	CalcSequenceLock func(*bteutil.Tx, *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error)
...
type TxPool struct {
	...
	orphansByPrev map[wire.OutPoint]map[chainhash.Hash]*bteutil.Tx
	outpoints     map[wire.OutPoint]*bteutil.Tx
...
func (mp *TxPool) removeOrphan(tx *bteutil.Tx, removeRedeemers bool) {
...
func (mp *TxPool) RemoveOrphan(tx *bteutil.Tx) {
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *bteutil.Tx, tag Tag) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		if _, exists := mp.orphansByPrev[txIn.PreviousOutPoint]; !exists {
			mp.orphansByPrev[txIn.PreviousOutPoint] =
				make(map[chainhash.Hash]*bteutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *bteutil.Tx, tag Tag) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeOrphanDoubleSpends(tx *bteutil.Tx) {
	msgTx := tx.MsgTx()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *bteutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
	if removeRedeemers {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *bteutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint, tx *bteutil.Tx, height int32, fee int64) *TxDesc {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *bteutil.Tx) (bool, error) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) signalsReplacement(tx *bteutil.Tx,
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txAncestors(tx *bteutil.Tx,
	cache map[chainhash.Hash]map[chainhash.Hash]*bteutil.Tx) map[chainhash.Hash]*bteutil.Tx {

	// If a cache was not provided, we'll initialize one now to use for the
	// recursive calls.
	if cache == nil {
		cache = make(map[chainhash.Hash]map[chainhash.Hash]*bteutil.Tx)
	}

	ancestors := make(map[chainhash.Hash]*bteutil.Tx)
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txDescendants(tx *bteutil.Tx,
	cache map[chainhash.Hash]map[chainhash.Hash]*bteutil.Tx) map[chainhash.Hash]*bteutil.Tx {
	...
	descendants := make(map[chainhash.Hash]*bteutil.Tx)
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txConflicts(tx *bteutil.Tx) map[chainhash.Hash]*bteutil.Tx {
	conflicts := make(map[chainhash.Hash]*bteutil.Tx)
	for _, txIn := range tx.MsgTx().TxIn {
		conflict, ok := mp.outpoints[txIn.PreviousOutPoint]
		if !ok {
			continue
...
func (mp *TxPool) CheckSpend(op wire.OutPoint) *bteutil.Tx {
	mp.mtx.RLock()
	txR := mp.outpoints[op]
	mp.mtx.RUnlock()
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *bteutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash) (*bteutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) validateReplacement(tx *bteutil.Tx,
	txFee int64) (map[chainhash.Hash]*bteutil.Tx, error) {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAcceptTransaction(tx *bteutil.Tx, isNew, rateLimit, rejectDupOrphans bool) ([]*chainhash.Hash, *TxDesc, error) {
	txHash := tx.Hash()
	...
	var conflicts map[chainhash.Hash]*bteutil.Tx
	if isReplacement {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *bteutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, *TxDesc, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(acceptedTx *bteutil.Tx) []*TxDesc {
	...
	for processList.Len() > 0 {
		...
		processItem := firstElement.(*bteutil.Tx)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *bteutil.Tx, allowOrphan, rateLimit bool, tag Tag) ([]*TxDesc, error) {
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphansByPrev:  make(map[wire.OutPoint]map[chainhash.Hash]*bteutil.Tx),
		nextExpireScan: time.Now().Add(orphanExpireScanInterval),
		outpoints:      make(map[wire.OutPoint]*bteutil.Tx),
	}
}
