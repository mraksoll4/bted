...
const (
	...
	DefaultMinRelayTxFee = bteutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee bteutil.Amount) int64 {
	...
	if minFee < 0 || minFee > bteutil.MaxSatoshi {
		minFee = bteutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *bteutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func IsDust(txOut *wire.TxOut, minRelayTxFee bteutil.Amount) bool {
...
func CheckTransactionStandard(tx *bteutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee bteutil.Amount,
...
func GetTxVirtualSize(tx *bteutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
