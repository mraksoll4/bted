...
type Config struct {
	...
	ProcessBlock func(*bteutil.Block, blockchain.BehaviorFlags) (bool, error)
...
func (m *CPUMiner) submitBlock(block *bteutil.Block) bool {
	...
	log.Infof("Block submitted via CPU miner accepted (hash %s, "+
		"amount %v)", block.Hash(), bteutil.Amount(coinbaseTx.Value))
	return true
}
...
out:
	for {
		...
		if m.solveBlock(template.Block, curHeight+1, ticker, quit) {
			block := bteutil.NewBlock(template.Block)
			m.submitBlock(block)
		}
	}
