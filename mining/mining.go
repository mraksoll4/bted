...
const (
	...
	MinHighPriority = bteutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr bteutil.Address) (*bteutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *bteutil.Tx, height int32) error {
...
func logSkippedDeps(tx *bteutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress bteutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*bteutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := bteutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := bteutil.NewBlock(&msgBlock)
...
func AddWitnessCommitment(coinbaseTx *bteutil.Tx,
	blockTxns []*bteutil.Tx) []byte {
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A bteutil.Block should use saved in the state to avoid
