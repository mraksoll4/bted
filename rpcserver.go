...
// list of commands that we recognize, but for which bted has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of btewallet.
...
// These commands will be implemented in btewallet.
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount*bteutil.SatoshiPerBitcoin > bteutil.MaxSatoshi {
			return nil, &btcjson.RPCError{
		...
		addr, err := bteutil.DecodeAddress(encodedAddr, params)
		if err != nil {
			return nil, &btcjson.RPCError{
		...
		satoshi, err := bteutil.NewAmount(amount)
		if err != nil {
...
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []btcjson.Vout {
	...
	for i, v := range mtx.TxOut {
		...
		vout.N = uint32(i)
		vout.Value = bteutil.Amount(v.Value).ToBTE()
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	txReply := &btcjson.TxRawResult{
		...
		Vsize:    int32(mempool.GetTxVirtualSize(bteutil.NewTx(mtx))),
		Weight:   int32(blockchain.GetTransactionWeight(bteutil.NewTx(mtx))),
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := bteutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blk, err := bteutil.NewBlockFromBytes(blkBytes)
	if err != nil {
...
// chainErrToGBTErrString converts an error returned from btechain to a string
...
func handleGetInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &btcjson.InfoChainResult{
		...
		RelayFee:        cfg.minRelayTxFee.ToBTE(),
	}
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	txOutReply := &btcjson.GetTxOutResult{
		...
		Value:         bteutil.Amount(value).ToBTE(),
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &btcjson.PrevOut{
				...
				Value:     bteutil.Amount(originTxOut.Value).ToBTE(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr bteutil.Address, numToSkip, numRequested uint32) ([]*bteutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	params := s.cfg.ChainParams
	addr, err := bteutil.DecodeAddress(c.Address, params)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
...
func handleSignMessageWithPrivKey(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	wif, err := bteutil.DecodeWIF(c.PrivKey)
	if err != nil {
		message := "Invalid private key"
		switch err {
		case bteutil.ErrMalformedPrivateKey:
			message = "Malformed private key"
		case bteutil.ErrChecksumMismatch:
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	addr, err := bteutil.DecodeAddress(c.Address, s.cfg.ChainParams)
	if err != nil {
	...
	default:
		// Handle the case when a new Address is supported by bteutil, but none
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if _, ok := addr.(*bteutil.AddressPubKeyHash); !ok {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCType,
	...
	address, err := bteutil.NewAddressPubKey(serializedPK, params)
	if err != nil {
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="bted RPC"`)
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *bteutil.Block, flags blockchain.BehaviorFlags) (bool, error)
